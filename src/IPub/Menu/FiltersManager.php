...
class FiltersManager implements \IteratorAggregate
{
	...
	{
		if (!is_string($filter) || !is_subclass_of($filter, 'IPub\Menu\Filter\FilterIterator')) {
			throw new Exceptions\InvalidArgumentException(sprintf('Given filter "%s" is not of type IPub\Menu\Filter\FilterIterator', (string) $filter));
		}
